...
`spring.cloud.loadbalancer.healthcheck`. You can set the `initialDelay`, `period` and `unit`
for the scheduler.

For the `PingHealthChecker`, you can set the default path for the healthcheck URL by setting
the value of the `spring.cloud.loadbalancer.healthcheck.path`. You can also set a specific value
for any given service by settin the value of the `spring.cloud.loadbalancer.[SERVICE_ID].healthcheck.path`,
